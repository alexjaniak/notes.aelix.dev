...
**Day 2 — Rent a GPU, stand up a real endpoint.** Vast/RunPod/Lambda (see [[GPU Rental Markets]]); one 4090 or A100. Install vLLM, serve an 8B model, hit it with the OpenAI client from the laptop. Save a reusable image/volume so future days start in minutes, not an hour. Post: "$1 and 40 minutes to my own OpenAI-compatible endpoint" + the cold-start-time reality vs [[Serving Models]] claims.
