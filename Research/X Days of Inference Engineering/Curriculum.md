...
**Day 3 — Benchmark it: watch continuous batching earn its keep.** → [[Day 3 — Benchmark the Endpoint]]. Sweep concurrency 1→128 with `vllm bench serve`, but derive everything first: knee from the decode roofline (arithmetic intensity ≈ B, ridge ~161 on A100), KV-corrected throughput curve, `num_gpu_blocks` from memory accounting, Little's law as per-row sanity check, goodput under a TTFT/TPOT SLO vs raw max. Server-side `/metrics` (running/waiting, cache %, preemptions) should show the knee before the client does. Also clears Day 2's debts: persistence, clean fused-sampler batch-1, the idle-KV `nvidia-smi` shot. Post: throughput-vs-concurrency chart, predicted vs actual knee.
