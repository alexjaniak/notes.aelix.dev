...
Research dump (July 2026) answering: *Where can I rent GPUs? How do you technically access a rental GPU? Can you write custom kernels without owning one?* From [[Questions]]. Related: [[GPUs]], [[Frontier Chips]]. Key citations verified July 2026.

## Where can I rent GPUs?

The market has stratified into ~5 layers with a 3–12x price spread for identical silicon (H100: ~$1.29/hr on marketplaces → $7–12/hr on hyperscalers; market median ~$2.45–3/hr). SemiAnalysis's [ClusterMAX 2.0](https://newsletter.semianalysis.com/p/clustermax-20-the-industry-standard) rating tracks **209 providers** (84 rated) — it's that crowded. Newest development: compute is becoming a listed commodity, with GPU futures launching on ICE and CME in 2026.
...
- **[Vast.ai](https://vast.ai/)** — the original "Airbnb of GPUs"; per-second billing, interruptible auctions, H100 from ~$1.33/hr, consumer cards from cents/hr; cheapest floor, reliability varies by host
- **[RunPod](https://www.runpod.io/)** — dev-focused hybrid: Community vs Secure tiers, serverless with sub-200ms FlashBoot cold starts, Instant Clusters
- **[SF Compute](https://sfcompute.com/)** — true order-book spot market for H100/H200 blocks; buy by the hour and *resell* unused time; owns no GPUs; [$40M Series A](https://www.datacenterdynamics.com/en/news/sf-compute-raises-40m-for-ai-compute-marketplace-offering/); positions as the physical base for compute derivatives
- **[Shadeform](https://www.shadeform.ai/)** (YC S23) — one API/console over 20–30+ providers at list prices, no markup
- **Prime Intellect** — compute exchange over 12+ clouds + decentralized training + RL environments hub; [$130M Series A @ $1B](https://techcrunch.com/2026/07/08/prime-intellect-raises-130m-series-a-to-help-enterprises-build-their-own-ai-agents/) (July 2026, Radical Ventures; NVentures, Intel Capital, Dell participating)
- **Mithril** (ex-Foundry) — spot/preemption arbitrage "omnicloud"; customers Cursor, LG, Arc Institute
- **[Hydra Host](https://hydrahost.com/blog/news/hydra-host-raises-100m-series-a/)** — bare-metal brokerage (Brokkr); $100M Series A (June 2026, Kindred Ventures, NVIDIA participating)
- **TensorDock** — vetted-host marketplace, full KVM VMs
- **[NVIDIA DGX Cloud Lepton](https://www.nvidia.com/en-us/data-center/dgx-cloud-lepton/)** — NVIDIA now runs its own rental marketplace routing developers to partner neocloud capacity
...
- **Ornn** — [GPU compute futures with ICE](https://ir.theice.com/press/news-details/2026/ICE-and-Ornn-to-Launch-GPU-Compute-Futures-Contracts/default.aspx) (May 2026; ["trade AI compute like oil"](https://www.axios.com/2026/07/06/ornn-gpu-compute-commodity))
- **Silicon Data** — SDH100RT H100 rental-price index; [first listed compute futures with CME Group](https://www.cmegroup.com/media-room/press-releases/2026/5/12/cme_group_and_silicondatapartnertolaunchfirstcomputefutures.html) (May 2026)
- **[Pluto](https://pluto.trade/)** — YC-backed, 4-person startup; aiming to be the "first CFTC-regulated exchange for AI infrastructure" (DCM/DCO applications filed, not yet approved)
...
1. **SSH into VM/bare metal** (Lambda, hyperscalers, CoreWeave dedicated) — provider preinstalls the NVIDIA driver; Lambda images ship drivers + CUDA + Jupyter. Standard workflow is VS Code/Cursor Remote-SSH. Hyperscaler gotcha: default GPU quota is 0, must request increases.
   - **Verified July 2026, Prime Intellect A100 80GB on-demand:** raw Ubuntu 22.04 VM (full `sudo apt` access, not a locked container), driver present (`nvidia-smi` reports the CUDA version the driver *supports*) but **no CUDA toolkit preinstalled** — `nvcc` genuinely missing, unlike Lambda's driver+CUDA+Jupyter baseline. Anything needing to JIT-compile a kernel at runtime (e.g. vLLM's FlashInfer fused sampler) will crash until you install a matching toolkit yourself. Also: your own SSH public key must be registered in the Prime Intellect dashboard *before* the instance is created (key injection is first-boot-only — a bad/late key means terminate + relaunch, not a fixable-in-place problem); a non-default-named local key isn't auto-offered by `ssh`, needs `-i` explicitly.
...
The one real constraint is **hardware-counter profiling** (Nsight Compute `ncu`): since driver 418, GPU perf counters are admin-restricted ([`ERR_NVGPUCTRPERM`](https://developer.nvidia.com/ERR_NVGPUCTRPERM)). You need bare metal, a dedicated VM, or `--cap-add=SYS_ADMIN`:
...
- **[LeetGPU](https://leetgpu.com/)** — browser CUDA problems + free simulator for zero-cost correctness checks
- **[Tensara](https://tensara.org/)** — competitive kernel challenges (CUDA/Triton/Mojo) on real T4/A100/H100, leaderboards vs PyTorch baselines
- **[GPU MODE](https://www.gpumode.com/)** Discord (~20k members) — free kernel submissions via [popcorn-cli](https://github.com/gpu-mode/popcorn-cli) to donated T4→B200/MI300 hardware, ~15s turnaround, weekly lectures; all 60k+ submitted kernels open-sourced
