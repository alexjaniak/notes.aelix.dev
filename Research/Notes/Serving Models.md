...
- **[S-LoRA](https://arxiv.org/abs/2311.03285)** (Berkeley, MLSys'24): unified memory pool for KV cache pages *and* adapter pages; 2,000 adapters served simultaneously on one GPU; 30x vs naive PEFT
- **[Punica](https://arxiv.org/abs/2310.18547)** (UW, MLSys'24): SGMV kernel batches decode across *different* LoRA models in one launch; 12x throughput, +2ms/token
- **[LoRAX](https://github.com/predibase/lorax)** (Predibase, open source; Predibase [acquired by Rubrik](https://www.rubrik.com/company/newsroom/press-releases/25/rubrik-to-acquire-predibase-to-accelerate-agentic-ai-adoption), June 2025): production multi-LoRA server — JIT adapter loading, heterogeneous continuous batching, async prefetch
- **vLLM built-in**: `--enable-lora`, GPU slots + CPU tier with LRU eviction, serves hundreds of adapters
- **Commercial**: [Together Serverless Multi-LoRA](https://www.together.ai/blog/serverless-multi-lora-fine-tune-and-deploy-hundreds-of-adapters-for-model-customization-at-scale) serves hundreds of custom adapters **at base-model per-token prices**; [Fireworks Multi-LoRA](https://fireworks.ai/blog/multi-lora) (now on dedicated/on-demand deployments rather than serverless); DeepInfra BYO-LoRA
- **Production case**: OpenPipe ([acquired by CoreWeave](https://www.coreweave.com/news/coreweave-to-acquire-openpipe-leader-in-reinforcement-learning), Sept 2025 → "Serverless RL" with W&B) cut cold-start 45s→1s and GPU count −70% via S-LoRA
...
**API level: trivial.** Everything is OpenAI-compatible — switching = change base URL + model string. Routers make it a config value: **OpenRouter** (500+ models, 70+ providers, ~25T tok/week, [$1.3B valuation](https://techcrunch.com/2026/05/26/openrouter-more-than-doubles-valuation-to-1-3b-in-a-year/); automatic fallbacks, `:floor`/`:nitro` suffixes), **Vercel AI Gateway**, **LiteLLM**, Portkey, Helicone. Workload-aware routing beats any single provider by 30–50% on cost.
...
- **Streaming loaders**: [NVIDIA Run:ai Model Streamer](https://developer.nvidia.com/blog/reducing-cold-start-latency-for-llm-inference-with-nvidia-runai-model-streamer/) (Llama-3-8B from S3 in 4.9s vs ~48s for the default safetensors loader), CoreWeave Tensorizer, vLLM instanttensor
- **GPU memory snapshots**: [Modal GPU snapshots](https://modal.com/blog/gpu-mem-snapshots) (vLLM boot 95.7s→13.8s) and NVIDIA Dynamo Snapshot, both built on the [cuda-checkpoint](https://github.com/NVIDIA/cuda-checkpoint) driver API — sub-second restores of loaded+compiled state
...
- Research: [ServerlessLLM](https://www.usenix.org/conference/osdi24/presentation/fu) (OSDI'24), FlashServe (0.58s cold start for 7B), Foundry (Qwen3-235B init 10min→3.9s)
...
5. **Orchestration** (multi-node) — **[llm-d](https://llm-d.ai/blog/llm-d-announce)** (Red Hat/Google/IBM-backed K8s-native, May 2025: prefix-cache-aware routing, tiered KV offload, KV-utilization autoscaling), **KServe** (lifecycle; its new LLMInferenceService CRD builds on llm-d), **[NVIDIA Dynamo](https://www.nvidia.com/en-us/ai/dynamo/)** (Triton's successor: KV-aware Smart Router, NIXL async KV transfer, tiered KV Block Manager), **Ray Serve**, **BentoML**
6. **KV infrastructure** — **[LMCache](https://github.com/LMCache/LMCache)** (tiered GPU→CPU→disk→remote KV reuse, [up to 15x with vLLM](https://arxiv.org/abs/2510.09665)), **[Mooncake](https://arxiv.org/abs/2407.00079)** (Moonshot's RDMA distributed KV store, FAST'25 best paper)
...
**vLLM is an inference engine** ([Berkeley, SOSP 2023](https://arxiv.org/abs/2309.06180)): takes weights and turns a GPU into a high-throughput OpenAI-compatible token service:
...
Headline: [up to 24x throughput vs raw HF Transformers](https://blog.vllm.ai/2023/06/20/vllm.html) (the paper's apples-to-apples claim vs older serving stacks like Orca/FasterTransformer is 2–4x). One reproduction: 220 tok/s naive → 980 (+continuous batching) → 3,100 (+PagedAttention) → 4,900 (+prefix caching) ≈ 22x.
...
- **SGLang** (LMSYS; commercialized by [RadixArk, $100M seed](https://www.radixark.com/blog/radixark-launches-100m-seed)) — **[RadixAttention](https://arxiv.org/abs/2312.07104)**: radix tree over ALL cached KV across requests → 70–90% prefill elimination on shared-prefix/agentic workloads; best for big MoE; powers xAI Grok and Azure DeepSeek-R1
